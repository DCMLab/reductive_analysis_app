...
export function do_relation(type, id, redoing = false) {
  ...
  if (selected.concat(extraselected)[0].classList.contains('relation')) {
    ...
    for (var i = 0; i < draw_contexts.length; i++) {
      let g_elem = draw_relation(
        draw_contexts[i],
        mei_graph,
        get_by_id(mei_graph.getRootNode(), he_id)
      );
      if (g_elem) {
...
export function do_metarelation(type, id, redoing = false) {
  ...
  var primaries = extraselected.map((e) => get_by_id(mei_graph.getRootNode(), id_or_oldid(e)));
  var secondaries = selected.map((e) => get_by_id(mei_graph.getRootNode(), id_or_oldid(e)));
  var [he_id, mei_elems] = add_metarelation(mei_graph, primaries, secondaries, type, id);
  added.push(mei_elems);
  for (var i = 0; i < draw_contexts.length; i++)
    added.push(
      draw_metarelation(draw_contexts[i], mei_graph, get_by_id(mei_graph.getRootNode(), he_id))
    ); // Draw the edge
...
function load_finish(loader_modal) {
  console.debug(
    'Using globals data, parser, mei, jquery document, document, mei_graph, midi, changes, undo_cations, redo_actions, reduce_actions, rerendered_after_action'
  );
  ...
  try {
    ...
    if (mei.getElementsByTagName('parsererror').length > 0) {
      console.log(
        'This is not a valid XML or MEI file. However it could be ABC or Humdrum, for instance'
      );
      // loader_modal.close()
    }
  } catch {
  ...
  for (let i in layers) {
    ...
    if (isFirstLayer) {
      ...
      orig_midi = midi;
    } else draw_context.id_prefix = draw_contexts.length;
...
export function create_new_layer(draw_context, sliced = false, tied = false) {
  var new_mdiv_elem;
  if (sliced) new_mdiv_elem = new_sliced_layer(draw_context, tied);
  else new_mdiv_elem = new_layer(draw_context);
