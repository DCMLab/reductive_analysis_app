...
function coord_staff(dc, pt, measure) {
  ...
  if (index_maybe == 0) return stave_coords[0][1];
  if (index_maybe == -1) return stave_coords[stave_coords.length - 1][1];
  const divider = average2(stave_coords[index_maybe - 1][0], stave_coords[index_maybe][0]);
  if (pt.y < divider) return stave_coords[index_maybe - 1][1];
  else return stave_coords[index_maybe][1];
}
...
export function pitch_grid(staff) {
  ...
  if (ns.length > 0) note = ns[0];
    ...
    const sibling_staff = staves.find(
      (st) => staff_midpoint(st) == mid && st.getElementsByClassName('note').length > 0
    );
    // And a note we can compare heights with
    if (sibling_staff) note = sibling_staff.getElementsByClassName('note')[0];
  }
  var mid_n;
  console.log(note);
  if (!note) {
    ...
    // For something that has the same height as this
    const sibling_staff = staves.find(
      (st) => staff_midpoint(st) == mid && st.getElementsByClassName('clef').length > 0
    );
...
function add_note(layer_context, pname, oct, note, sim = true, id = '') {
  ...
  if (sim) {
    let c;
    if (l.closest('chord')) c = l.closest('chord');
    else {
...
export function do_note(pname, oct, note, offset, id, redoing = false) {
  ...
  let n = note;
  if (typeof id != 'undefined') new_element_id = id;
  var added = [];
  // Draw it temporarily
  console.log(note);
