...
function delete_relation(elem) {
  ...
  for (const draw_context of draw_contexts) {
    ...
    if (svg_he) {
      svg_hes.push(svg_he);
      if (!metarel) unmark_secondaries(draw_context, mei_graph, mei_he);
    }
  }

  var arcs = Array.from(mei.getElementsByTagName('arc')).filter((arc) => {
    return arc.getAttribute('to') == '#' + elem.id || arc.getAttribute('from') == '#' + elem.id;
  });
...
export function delete_relations(redoing = false) {
  console.debug(
    'Using globals: selected for element selection, undo_actions for storing the action'
  );
  // Assume no meta-edges for now, meaning we only have to
  ...
  if (
    sel.length == 0 ||
    !(
      get_class_from_classlist(sel[0]) == 'relation' ||
