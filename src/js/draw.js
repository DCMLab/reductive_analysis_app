...
export function draw_relation(draw_context, mei_graph, g_elem) {
  ...
  var primaries = relation_primaries(mei_graph, g_elem).map((e) =>
    document.getElementById(id_in_svg(draw_context, node_to_note_id(e)))
  );
  var secondaries = relation_secondaries(mei_graph, g_elem).map((e) =>
    document.getElementById(id_in_svg(draw_context, node_to_note_id(e)))
  );
  ...
  if (id_prefix != '') elem.setAttribute('oldid', g_elem.getAttribute('xml:id'));
  ...
  toggle_shade(elem);
  if (!newApp.ui.scoreSettings.brightShades) toggle_shade(elem);
...
export function draw_metarelation(draw_context, mei_graph, g_elem) {
  ...
  var targets = relation_allnodes(mei_graph, g_elem).map((e) =>
    document.getElementById(draw_context.id_prefix + get_id(e))
  );
  var primaries = relation_primaries(mei_graph, g_elem).map((e) =>
    document.getElementById(id_in_svg(draw_context, node_to_note_id(e)))
  );
  ...
  var y =
    targets
      .concat([svg_elem.getElementsByClassName('system')[0]])
      .map((b) => b.getBBox().y)
      .sort((a, b) => a > b)[0] - 500;
  ...
  toggle_shade(g_elem);
  if (!newApp.ui.scoreSettings.brightShades) toggle_shade(g_elem);

  // We can scroll among metarelations as well
  g_elem.addEventListener(
    'wheel',
    (e) => {
      e.preventDefault();
  ...
  g_elem.onmouseover = function (ev) {
    primaries.forEach((item) => {
      if (item.classList.contains('relation')) item.classList.add('extrarelationhover');
      else item.children[0].classList.add('extrarelationhover');
    });
    secondaries.forEach((item) => {
      if (item.classList.contains('relation')) item.classList.add('relationhover');
      else item.children[0].classList.add('relationhover');
    });
  };
  g_elem.onmouseout = function (ev) {
    primaries.forEach((item) => {
      if (item.classList.contains('relation')) item.classList.remove('extrarelationhover');
      else item.children[0].classList.remove('extrarelationhover');
    });
    secondaries.forEach((item) => {
      if (item.classList.contains('relation')) item.classList.remove('relationhover');
      else item.children[0].classList.remove('relationhover');
    });
  };
