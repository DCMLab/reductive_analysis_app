...
function layer_clone_element(changes, elem, new_children) {
  const no_notes_below =
    new_children.findIndex(
      (e) =>
        e.nodeType == Node.ELEMENT_NODE &&
        (e.tagName == 'note' || e.getElementsByTagName('note').length > 0)
    ) == -1;
  // No need to copy over empty beams and measures.
  if ((elem.tagName == 'beam' || elem.tagName == 'measure') && no_notes_below) return null;
  // Replace empty chords with spaces
  if (elem.tagName == 'chord' && no_notes_below) return chord_to_space(mei, elem);
...
function layerify(draw_context, elem) {
  if (elem.nodeType != Node.ELEMENT_NODE) return [elem.cloneNode(), false];
  var svg_elem = document.getElementById(id_in_svg(draw_context, get_id(elem)));
  if (elem.tagName == 'note' && (!svg_elem || svg_elem.classList.contains('hidden')))
    // This elem has been reduced away
  ...
  var changes =
    new_children.length != elem.childNodes.length || // Something directly below was reduced
...
export function mei_for_layer(mei, mdiv_elem) {
  ...
  var our_mdiv;
  if (!mdiv_elem.hasAttribute('xml:id')) our_mdiv = new_mei.getElementsByTagName('mdiv')[0];
  else our_mdiv = get_by_id(new_mei, mdiv_elem.getAttribute('xml:id'));
  var paren = our_mdiv.parentElement;
  for (let mdiv of Array.from(paren.getElementsByTagName('mdiv'))) {
    if (mdiv === our_mdiv) continue;
    else paren.removeChild(mdiv);
  }
  return new_mei;
}
