  ...
  drag_selector.subscribe('DS:update', e => {
    ...
    if (placing_note == '') {
      ...
      document
        ...
        .map(x => {
          switch (x.tagName) {
            // Selecting relation by the path
            case 'path':
        ...
        .filter(x => {
          let view_number = getViewNumberForElement(x)
          if (typeof x == 'undefined' || typeof x.classList == 'undefined' || view_number !== '0') return false
          const drawContexts = getDrawContexts()
          if (!drawContexts) return null
          const topmost_context = drawContexts.find(c => c?.layer?.layer_elem?.id === 'layer0') || null
          if (topmost_context) setCurrentDrawContext(topmost_context)
...
/**
 * Get the view ID number of an HTML element, returned as string.
 * (This is "5" in, for example, id="view5")
 */
