...
class Score {
  ...
  get selectedRelationTypes() {
    return this.#cache.remember('selectedRelationTypes', () => {
      if (!this.hasSelection || this.selectionType == 'note') {
        return null;
      }

      return new Set(
        this.flatSelection.map((el) => el.getAttribute('type')).filter((type) => type)
      );
    });
  }
  ...
  onTap(e) {
    if (!this.loaded || !e.composedPath().includes(this.layersCtn)) {
      return;
    }
