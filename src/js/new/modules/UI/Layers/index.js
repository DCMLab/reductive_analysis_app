...
class LayersMenu {
  ...
  onTap(e) {
    ...
    if (e.target == this.nextLayerBtn) {
      return this.moveBy(1);
    }
    if (e.target == this.previousLayerBtn) {
      return this.moveBy(-1);
    }

    if (e.target == this.$lockBtn) {
      return this.toggleLock();
    }
    if (e.target == this.$shouldSave) {
    ...
    this.contexts
      ...
      .forEach((layer) => {
        ...
        if (layer.layer.layer_elem.childElementCount == 1) {
          layer.layer.layer_elem.insertAdjacentHTML(
            'beforeend',
    ...
    this.observer = new IntersectionObserver(
      (entries) => {
        ...
        entries.forEach((entry) => {
          const layer = this.contexts.find(
            (layer) => entry.target.dataset.position == layer.layer.layer_number
          );
          if (layer) {
        ...
        const mostVisibleLayer = this.contexts.find(
          (layer) => layer.visibleHeight == highestVisibility
        );
  ...
  addMouseListeners() {
    if (!navigation_conf.switch_context_on_hover) {
      return;
    }
    ...
    if (this.contexts.length < 2) {
      ...
      return this.contexts[0].layer.layer_elem.removeEventListener(
        'mouseenter',
        this.markAsCurrent.bind(this)
      );
    }
