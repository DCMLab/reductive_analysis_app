...
export default class JsonTree {
  ...
  onTap({ target }) {
    ...
    if (target == this.saveBtn) {
      return save_tree(currentLayerObject);
    }
    if (target == this.loadBtn) {
      return load_tree(currentLayerObject);
    }
    if (target == this.alignBtn) {
      return align_tree(currentLayerObject);
    }
  }

  onSubmit(e) {
    if (e.target.id != 'json-tree-form') {
      return;
    }
