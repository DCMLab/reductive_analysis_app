import { createBtn, createDatalistFillable, createShowMoreBtn, createTitle } from './templates';
...
export default class RelationsGroup {
  ...
  unselect() {
    // Unselect buttons
    Array.from(this.selectedBtns).forEach((btn) => btn.classList.remove('btn--selected'));
  ...
  initHtml(config) {
    ...
    const additionalRelations =
      'additional' in config ? createDatalistFillable(config, this.type) : '';
