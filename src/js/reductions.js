...
export function calc_reduce(mei_graph, remaining_relations, target_relations) {
  ...
  // reduction
  var remaining_nodes = remaining_relations.map((he) => relation_primaries(mei_graph, he)).flat();
  ...
  remaining_nodes = remaining_nodes.concat(
    remaining_relations.map((he) => relation_secondaries(mei_graph, he)).flat()
  );
  ...
  do {
    ...
    remaining_nodes = remaining_nodes.concat(
      more_remains.map((he) => relation_secondaries(mei_graph, he)).flat()
    );
  ...
  return [
    target_relations,
    [...new Set(target_relations.flatMap((he) => relation_secondaries(mei_graph, he)))],
  ];
}
...
function do_reduce(draw_context, mei_graph, sel, extra) {
  ...
  var target_relations = selection.map((ge) => get_by_id(mei_graph.getRootNode(), get_id(ge)));

  var all_relations_nodes = Array.from(mei_graph.getElementsByTagName('node')).filter(
    (x) => x.getAttribute('type') == 'relation'
  );
  ...
  if (target_relations.length == 0) target_relations = remaining_relations;

  // The removed notes we get are _nodes in the graph_, but
  // hide_note is built with that in mind.
  var [removed_relations, removed_notes] = calc_reduce(
    mei_graph,
    remaining_relations,
    target_relations
  );
  var graphicals = [];
  graphicals.push(removed_relations.map((r) => hide_he(draw_context, r)));

  graphicals.push(removed_notes.map((n) => hide_note(draw_context, n)));
  graphicals.push(removed_relations.map((r) => hide_he_hier(draw_context, r)));
  graphicals.push(removed_notes.map((n) => hide_note_hier(draw_context, n)));
