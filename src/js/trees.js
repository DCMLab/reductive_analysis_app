...
function adjust_y(node, parent_y, min_dist) {
  ...
  if (Math.abs(parent_y - node.y) > min_dist) node.y += (parent_y - min_dist - node.y) / 2;
...
function calculate_x(node) {
  ...
  if (node.children.length == 0 || node.x != undefined) return;
...
function draw_node(node, fontSize = 100) {
  ...
  var node_text = text(node.label, [node.x, node.y + (node.children.length == 0 ? fontSize : 0)]);
...
function obj_tree_to_xml(json_tree) {
  ...
  if (elems) elems.forEach((e) => elem.appendChild(e));
...
function add_or_replace_tree(elem) {
  ...
  if (pNode.querySelector('eTree')) pNode.removeChild(pNode.querySelector('eTree'));
...
export function align_tree(draw_context) {
  ...
  if (selected.length == 0) {
    ...
    if (selected[0].classList.contains('relation'))
      notelist = relation_get_notes(selected[0]).map((n) =>
        get_by_id(document, id_in_svg(draw_context, n.getAttribute('xml:id')))
      );
...
export function draw_tree(draw_context, baseline = 0, min_dist = -1000) {
  ...
  if (!has_x_tree(obj)) find_x_tree(draw_context, obj);
