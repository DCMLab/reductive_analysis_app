...
export function do_undo() {
  console.debug(
    'Using globals: undo_actions, selected, extraselected, mei, rerendered_after_action'
  );
  ...
  if (what == 'edges' || what == 'relation' || what == 'metarelation') {
    ...
    let id,
      type,
      g_elem = added.flat().find((e) => e.tagName == 'node' && e.getAttribute('type') == what);
    if (g_elem) {
      id = g_elem.getAttribute('xml:id');
      if (g_elem.children.length > 0) type = g_elem.children[0].getAttribute('type');
    } else {
    ...
    if (what == 'relation')
      for (const dc of draw_contexts) unmark_secondaries(dc, getMeiGraph(), g_elem);
    // Remove added elements
    added.flat().forEach((x) => {
      if (!node_referred_to(x.getAttribute('xml:id'))) x.parentNode.removeChild(x);
    });
    // Find and remove any leftover graphical elements
    Array.from(document.querySelectorAll('[oldid="' + id + '"]')).forEach((x) =>
