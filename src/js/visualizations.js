...
function calc_hierarchy(notes, relations, roots_low = true) {
  ...
  do {
    ...
    var [removed_relations, removed_notes] = calc_reduce(mei_graph, rels, rels);
...
export function draw_hierarchy_graph(draw_context, hullPadding = 200, roots_low = true) {
  ...
  // find layers
  var current_note_nodes = Array.from(svg_elem.getElementsByClassName('note'))
    .map(get_id)
    .map((id) => get_by_id(mei, id))
    .map(get_id)
    .map((id) => get_by_id(mei, 'gn-' + id))
    .filter((x) => x != undefined);
  var current_relation_nodes = Array.from(svg_elem.getElementsByClassName('relation'))
    .map(get_id)
    .map((id) => get_by_id(mei, id));
  ...
  // find coordinates
  var layers_coords = layers.flatMap((layer, ix) =>
    layer.map((e) => {
  ...
  layers_coords.forEach(([e, p]) => {
    ...
    } else txt = note_g.getElementsByTagName('text')[0];
